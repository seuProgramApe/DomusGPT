...
class RoleContext:
    def __init__(self):
        """角色运行时上下文."""
        self.env = None  # 在角色被添加到Supervisor的环境中时，将该环境设置为角色的环境.
        self.memory = Memory()
        self.state = 0
        self.todo = None
        self.watch = set()
...
class Role:
    ...
    def _set_state(self, state):
        ...
        self._rc.todo = self._actions[
            self._rc.state
        ]  # 通过self._think()方法获得的下一个state，然后将self._rc.todo设置为self._actions[state]

    def set_env(
        self, env
    ):  # 当role被添加到jarvis的环境中时，调用这个方法，将jarvis的环境设置为role的环境
    ...
    async def _act(self, msg: Message) -> Message:
        ...
        if self.profile not in rsp_msg.send_to:
            ...
            # 不将发给自己（调用工具导致）的信息在此处加入运行时上下文，因为这些信息应该作为出现在环境中的新信息在_observe中被加入记忆并被响应。
            # 否则将回复信息加入自己的上下文
        return rsp_msg
    ...
    async def _observe(self) -> int:
        ...
        received = []
        received = [
            i for i in env_msgs if self.profile in i.send_to
    ...
    async def run(self, message=None):
        ...
        new_message = await self._observe()  # 获取最新消息
        if new_message:
        ...
        return (self.profile, rsp_message)
