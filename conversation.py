"""Defines the SmartHomeAgent Conversation backend."""
...
from .const import DOMAIN, CONF_PROVIDER, PROVIDERS, DEFAULT_PROVIDER, WORK_PATH
...
class LocalLLMAgent(ConversationEntity, AbstractConversationAgent):
    ...
    async def async_process(self, user_input: ConversationInput) -> ConversationResult:
        ...
        if conversation_id not in self.history:  # 李安：history: dict[str, list[dict]]
...
class GenericOpenAIAPIAgent(LocalLLMAgent):
    ...
    async def _async_load_model(self, entry: ConfigEntry) -> None:
        ...
        if os.path.exists(
            "/workspaces/hahaha/config/.storage/chatiot_conversation/temp"  # 这里正式发布时需要修改
        ):
    ...
    async def _async_generate(
        self, conversation_id: str, conversation: list[dict]
    ) -> str:
        try:
            result = await self.jarvis.run(conversation[-1]["message"])
            # print("_async_generate() result:" + result)
