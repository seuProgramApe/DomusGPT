...
def STEP_CONFIG_PROVIDER_SCHEMA(
    provider=None,
    api_key=None,
    base_url=None,
    temperature=None,
    ...
    return vol.Schema(
        {
            vol.Required(
                CONF_PROVIDER, default=provider if provider else DEFAULT_PROVIDER
            ): SelectSelector(
                SelectSelectorConfig(
                    options=PROVIDERS, multiple=False, mode=SelectSelectorMode.DROPDOWN
                )
            ),
            vol.Required(
                CONF_API_KEY, default=api_key if api_key else DEFAULT_API_KEY
            ): str,
            vol.Required(
                CONF_BASE_URL, default=base_url if base_url else DEFAULT_BASE_URL
            ): str,
            vol.Required(
                CONF_TEMPERATURE,
                default=temperature if temperature else DEFAULT_TEMPERATURE,
            ): NumberSelector(NumberSelectorConfig(min=0.0, max=1.0, step=0.1)),
            vol.Required(
                CONF_MAX_TOKENS,
                default=max_tokens if max_tokens else DEFAULT_MAX_TOKENS,
            ): NumberSelector(NumberSelectorConfig(min=1, max=2048, step=1)),
            vol.Required(
                CONF_ACCESS_TOKEN,
    ...
    async def _async_validate_access_token(self, user_input) -> tuple:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                _logger.error(
                    f"Failed to connect to Home Assistant server: {response.status_code}: {response.text}"
                )
        ...
        if user_input:
            ...
            try:
                ...
                (
                    error_message_llm,
                    exception_llm,
                ) = await self._async_validate_generic_openai(user_input)
                ...
                (
                    error_message_hass,
                    exception_hass,
                ) = await self._async_validate_access_token(user_input)
                ...
                if error_message_llm or error_message_hass:
                    ...
                    if exception_llm or exception_hass:
                        ...
                        description_placeholders["exception"].append(
                            str(exception_hass)
                        )
                    schema = STEP_CONFIG_PROVIDER_SCHEMA(
